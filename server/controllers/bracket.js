...
module.exports.scoreDisplayPage = (req, res, next) => {
  ...
  if (id2 <= Math.ceil(player/2)){//gets round #, scalable
    id3 = 1;
  }else if (id2 <= (Math.ceil(player/2) + Math.ceil(player/4))){
    id3 = 2;
  }else if (id2 <= (Math.ceil(player/2) + Math.ceil(player/4) + Math.ceil(player/8))){
    id3 = 3;
  }else{
    id3 = 4;
  }
...
module.exports.scoreProcessPage = async (req, res, next) => {
  ...
  if (id3 == 1){//round changes which score array is required
  ...
  scorVal2.slice(scorVal2.indexOf("'"), scorVal2.lastIndexOf("'", scorVal2.indexOf("'")+1));//the actual values are taken from the score array 
  
  let winVal2 = winVal3.split(",");
  for (var c = 1; c < winVal2.length; c++){//removes line breaks for the whole winVal array 
    while(winVal2[c].includes("\n")){
      ...
      if(winVal2[c].indexOf("\n")>1){
        ...
        winVal2[c] = (winVal2[c].slice(0, (winVal2[c].indexOf("\n")/*-1*/)))+(winVal2[c].slice((winVal2[c].indexOf("\n")+1/*+1*/), winVal2[c].length));
      }else{
        winVal2[c] = winVal2[c].slice((winVal2[c].indexOf("\n"))+1/*+2*/, winVal2[c].length);
      }
    }
    ...
    while((winVal2[c].includes("[")) || winVal2[c].includes("]")){
      if(winVal2[c].includes("[")){
        winVal2[c] = (winVal2[c].slice(0, (winVal2[c].indexOf("["))))+(winVal2[c].slice((winVal2[c].indexOf("[")+1), winVal2[c].length));
      }else if (winVal2[c].includes("]")){
        winVal2[c] = (winVal2[c].slice(0, (winVal2[c].indexOf("]"))))+(winVal2[c].slice((winVal2[c].indexOf("]")+1), winVal2[c].length));
      }
    }
  }
  if(winVal2[1].includes("winner:")){//error handling, to fix a bug
    winVal2[1] = winVal2[1].slice( (7+winVal2[1].indexOf("winner:")), winVal2[1].length);
    //console.log("Error, contained 'winner: [' now reads: " + winVal2[1]);
  }
  let winVal = [];
  let scorVal3 = [];
  for (var c = 0; c < scorVal2.length; c++){//starts at 1 because first line is _id
    if ((scorVal2[c] !== undefined)){// || ((scorVal2[(c-1)].length)>0)){
      scorVal3[c] = scorVal2[c];//.slice(1,-1).trim();
    }
  }
  for (var c = 0; c < winVal2.length; c++){
    if (winVal2[c] !== undefined){//trims out the values in the winval array
      if (winVal2[c].includes("'")){
        //console.log(c + " w ' 1: " + winVal2[c].indexOf("'") + " w ' 2: " + winVal2[c].lastIndexOf("'"));
        winVal2[c] = winVal2[c].trim();
        winVal[c-1] = winVal2[c].slice((winVal2[c].indexOf("'")+1),(winVal2[c].lastIndexOf("'")));
      }else if(c != 0 ){
  ...
  //winner calculation variables
    ...
    if(id3 == 1){//round 1 calcs
      for (var c = 0; c<half; c++){//c = 0 -> 3 or 0 -> 7
        ...
        }else if ((scorVal3[b] === undefined) || (scorVal3[b2] === undefined) || (scorVal3[b] == "") || (scorVal3[b2] == "") ){//error handling returns the output to the starting output
          winVal[c] = "Game 1";
        }else if ((scorVal3[b] == "Game 1") || (scorVal3[b] == "Game 2") ||(scorVal3[b] == "Game 3") ||(scorVal3[b] == "Final") ){
          winVal[c] = "Game 1";
        }else if ((scorVal3[b2] == "Game 1") || (scorVal3[b2] == "Game 2") ||(scorVal3[b2] == "Game 3") ||(scorVal3[b2] == "Final") ){
          winVal[c] = "Game 1";
       } else{
      ...
      for (var c = half; c<(half+qurt); c++){//c = 4 -> 6 or 8 -> 12
        ...
        //commented out sections for double checking calculations 
                ...
                //console.log(scorVal3[b] + " <[b] + c:" +c + " [b2]> " + scorVal3[b2]);
        if (scorVal3[b] > scorVal3[b2]){//same block as the previous section
          winVal[c] = b; 
        }else if (scorVal3[b] < scorVal3[b2]){
          winVal[c] = b2; 
        } else if ( scorVal3[b] = scorVal3[b2] ){
          winVal[c] = "Tie Game";
        }else if ((scorVal3[b] === undefined) || (scorVal3[b2] === undefined) || (scorVal3[b] == '') || (scorVal3[b2] == '') || (scorVal3[b] == 'Game 1') || (scorVal3[b2] == 'Game 1')){
    ...
    }else if(id3 == 3){//round 3 calcs
      for (var c = 0; c < half+qurt; c++){
        winVal[c] = winVal[c];
      }
      //console.log("First 3/4 unchanged");
      for (var c = (half+qurt); c<(half+qurt+eigh); c++){//c = 6 -> 7 or 13 -> 14
        ...
        if (scorVal3[b] > scorVal3[b2]){
          ...
          winVal[c] = "Tie Game";
        }else if ((scorVal3[b] === undefined) || (scorVal3[b2] === undefined) || (scorVal3[b] == '') || (scorVal3[b2] == 'Game 2') || (scorVal3[b] == '') || (scorVal3[b2] == 'Game 2')){//
          console.log("Probable error, outputing Game 2");
      ...
      for (var c = 0; c < 14; c++){//constants instead of variables for final of 16
      ...
      if (scorVal3[b] > scorVal3[b2]){
        ...
        winVal[c] = "Tie Game"; 
      }else if ((scorVal3[b] === undefined) || (scorVal3[b2] === undefined) || (scorVal3[b] == '') || (scorVal3[b2] == '')  || (scorVal3[b] == 'Game 3') || (scorVal3[b2] == 'Game 3') ){//
        winVal[c] = winVal[c];
      }else{
        winVal[c] = "Tie Game";
      }
  ...
  let newScore;
  if (id3 == 1){//new score reflects which scoreG has been altered
    newScore = Bracket({
      _id: id,
      teams: req.body.teams,
  ...
  Bracket.updateOne({_id:id }, newScore, (err) => {//update is made to the database
    if (err) {
      console.log(err);
      res.end(err);
    } else {
